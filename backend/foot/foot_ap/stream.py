...
st.subheader("📸 Axoris Adaptive AP Leg Positioner")
st.info("🎥 Status: Optimized for Leg Alignment and Perspective Calibration Locks.")
...
target_leg = st.sidebar.selectbox("🦵 Target Leg for Capture", ["Right Leg", "Left Leg", "Both Legs"])
...
engine_right = AdaptiveLegStabilizer(window_size=12, box_alpha=0.05)
engine_left = AdaptiveLegStabilizer(window_size=12, box_alpha=0.05)

base_options = python.BaseOptions(model_asset_path=MODEL_PATH)
options = vision.PoseLandmarkerOptions(
    base_options=base_options, running_mode=vision.RunningMode.VIDEO, num_poses=1,
    min_pose_detection_confidence=0.45, min_tracking_confidence=0.45        
)
    ...
    if vs.frame is None:
        ...
        while vs.started:
            ...
            if landmarks is not None:
                # Mapped left body landmarks (23, 25, 27) -> Right Leg (appears on the left side of screen)
                # Mapped right body landmarks (24, 26, 28) -> Left Leg (appears on the right side of screen)
                
                # Check visibility for Right Leg (landmarks 23, 25, 27)
                right_leg_visible = (landmarks[23].visibility > 0.25 and 
                                     landmarks[25].visibility > 0.25 and 
                                     landmarks[27].visibility > 0.25)
                
                # Check visibility for Left Leg (landmarks 24, 26, 28)
                left_leg_visible = (landmarks[24].visibility > 0.25 and 
                                    landmarks[26].visibility > 0.25 and 
                ...
                if right_leg_visible:
                    ...
                    angle_ok = 172.0 <= angle <= 180.0
                    
                    # Leaning check (deviation from vertical - strict 8 degrees threshold)
                    leaning_angle = np.degrees(np.arctan2(abs(ankle_pt[0] - hip_pt[0]), abs(ankle_pt[1] - hip_pt[1]) + 1e-6))
                    leaning_ok = leaning_angle <= 8.0
                ...
                rotation_ok = True
                facing_front_ok = True
                hip_tilt_ok = True
                right_sagittal_ok = True
                left_sagittal_ok = True
                
                if both_visible:
                    # Body rotation check (hips rotation around Y axis) - relaxed threshold
                    dx = landmarks[23].x - landmarks[24].x
                    dz = landmarks[23].z - landmarks[24].z
                    rotation_angle = np.degrees(np.arctan2(abs(dz), abs(dx) + 1e-6))
                    rotation_ok = rotation_angle <= 35.0
                    
                    # Backward pose check (toes must be in front of heels along the Z axis) - relaxed threshold
                    right_facing_front = landmarks[32].z < landmarks[30].z + 0.15
                    left_facing_front = landmarks[31].z < landmarks[29].z + 0.15
                    facing_front_ok = right_facing_front and left_facing_front
                    
                    # Hip tilt check (pelvic tilt) - relaxed threshold
                    hip_dy = landmarks[23].y - landmarks[24].y
                    hip_dx = landmarks[23].x - landmarks[24].x
                    hip_tilt_angle = np.degrees(np.arctan2(abs(hip_dy), abs(hip_dx) + 1e-6))
                    hip_tilt_ok = hip_tilt_angle <= 12.0
                    
                    # Sagittal leaning check (deviation of hip from ankle along Z axis) - relaxed threshold
                    right_sagittal_lean = np.degrees(np.arctan2(abs(landmarks[24].z - landmarks[28].z), abs(landmarks[24].y - landmarks[28].y) + 1e-6))
                    left_sagittal_lean = np.degrees(np.arctan2(abs(landmarks[23].z - landmarks[27].z), abs(landmarks[23].y - landmarks[27].y) + 1e-6))
                    right_sagittal_ok = right_sagittal_lean <= 30.0
                    left_sagittal_ok = left_sagittal_lean <= 30.0

                # Non-target leg thresholds are slightly more relaxed to avoid false negatives
                right_non_target_ok = right_state["visible"] and (168.0 <= right_state["angle"] <= 180.0) and right_state["leaning_ok"]
                left_non_target_ok = left_state["visible"] and (168.0 <= left_state["angle"] <= 180.0) and left_state["leaning_ok"]
                ...
                global_ok = (
                    ...
                    right_state["angle_ok"] and
                    right_state["leaning_ok"] and
                    right_state["motion_ok"] and
                    left_state["angle_ok"] and
                    left_state["leaning_ok"] and
                ...
                if target_leg == "Right Leg":
                    if not both_visible:
                        ...
                        if final_ok:
                            ...
                            color = (0, 255, 0)
                            status_msg.success(f"✅ RIGHT LEG LOCKED: Alignment Validated ({leg_angle:.1f}°).")
                        else:
                            ...
                            if not facing_front_ok:
                                feedback_details.append("Avoid facing away from camera.")
                            if not hip_tilt_ok:
                                feedback_details.append(f"Avoid tilting hips ({hip_tilt_angle:.1f}°/12.0°).")
                    ...
                    if not both_visible:
                        ...
                        if final_ok:
                            ...
                            color = (0, 255, 0)
                            status_msg.success(f"✅ LEFT LEG LOCKED: Alignment Validated ({leg_angle:.1f}°).")
                        else:
                ...
                cv2.putText(display_frame, f"TARGET: {target_leg.upper()}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
