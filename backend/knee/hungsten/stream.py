...
st.subheader("📸 Patella Tangential Live Positioner (Hughston Method)")
...
class FallbackBufferStabilizer:
    def __init__(self, alpha_high=0.8, alpha_low=0.4, window_size=5):
    ...
    def smooth_box(self, current_corner):
        if self.last_box is None:
            ...
            # Responsive box center smoothing (alpha = 0.5)
            self.last_box = (
                int(0.5 * current_corner[0] + 0.5 * self.last_box[0]),
                int(0.5 * current_corner[1] + 0.5 * self.last_box[1])
            )
...
options = vision.PoseLandmarkerOptions(
    ...
    min_pose_detection_confidence=0.20, min_tracking_confidence=0.45        
)
    ...
    if vs.frame is None:
        ...
        while vs.started:
            ...
            if result.pose_landmarks:
                ...
                consecutive_drops += 1
                # Hard reset cached pose landmarks after 5 consecutive failed detections (~165ms)
            ...
            if last_landmarks is not None:
                ...
                if left_score > right_score:
                    ...
                    side_label = "TANGENTIAL PATELLA: LEFT LOWER LIMB"
                else:
                    hip_lm, knee_lm, ankle_lm = landmarks[24], landmarks[26], landmarks[28]
                    side_label = "TANGENTIAL PATELLA: RIGHT LOWER LIMB"
                ...
                # ==========================================
                # ✅ FIXED HUGHSTON GEOMETRIC ANALYSIS LAW
                # ==========================================
                # Vector representing the Tibia/Fibula segment (Knee -> Ankle)
                tibia_vector = np.array([ankle_pt[0] - knee_pt[0], ankle_pt[1] - knee_pt[1]])
                
                # Baseline horizontal vector representing the flat table surface plane
                table_vector = np.array([1, 0])
                
                # Direct dot product computation ensuring measurement stays locked to the table baseline
                dot_prod = np.dot(tibia_vector, table_vector)
                mag_tibia = np.linalg.norm(tibia_vector) + 1e-6
                cosine_angle = dot_prod / mag_tibia
                
                # Calculate the clear physical angle relative to the table surface plane
                tibia_table_angle = np.degrees(np.arccos(np.clip(abs(cosine_angle), -1.0, 1.0)))
                
                # Clinical criteria gate window check (45° to 70° target range)
                angle_ok = 43.0 <= tibia_table_angle <= 72.0

                # Rule 2: Prone Verification Check (Hip must remain lower or level with the knee)
                is_prone = hip_pt[1] >= knee_pt[1] - 15

                # Rule 3: Leg Raised Check (Ankle must be physically raised above the knee by at least 35% of the tibia length)
                leg_raised = (knee_pt[1] - ankle_pt[1]) >= (pixel_scale * 0.35)

                # Rule 4: Knee Flexion Angle Check (Knee joint must be bent, not straight or over-flexed)
                knee_joint_angle = calculate_angle_3pt(hip_pt, knee_pt, ankle_pt)
                knee_bent = 95.0 <= knee_joint_angle <= 145.0

                good_posture = angle_ok and is_prone and leg_raised and knee_bent
                ...
                if out_of_bounds:
                    ...
                    if good_posture:
                        ...
                        color = (0, 255, 0)
                        status_msg.success(f"✅ POSITION PERFECT: Tibia Flexion Angle at {tibia_table_angle:.1f}° (Target: 45°-70°)")
                    else:
                        status_text = "WRONG POSTURE"
                        color = (0, 0, 255)
                        if not is_prone:
                            feedback_details.append("Lie flat on stomach in a true prone position")
                        else:
                            if not leg_raised:
                                feedback_details.append("Bend your knee to raise your foot up off the bed")
                            elif not knee_bent:
                                if knee_joint_angle > 145.0:
                                    feedback_details.append("Bend your knee more to raise the foot")
                                elif knee_joint_angle < 95.0:
                                    feedback_details.append("Push foot further away (knee bent too much)")
                            elif not angle_ok:
                                if tibia_table_angle < 43.0:
                                    feedback_details.append(f"Pull foot closer to buttock ({tibia_table_angle:.1f}° found, target 45°-70°)")
                                else:
                                    feedback_details.append(f"Push foot further away down ({tibia_table_angle:.1f}° found, target 45°-70°)")
                ...
                if 'global_telemetry' in globals():
                    if out_of_bounds:
                        ...
                        global_telemetry['accuracy'] = 45
                        if not is_prone:
                            global_telemetry['message'] = "Warning: Lie flat on stomach in a true prone position"
                        elif not leg_raised:
                            global_telemetry['message'] = "Warning: Bend your knee to raise your foot up"
                        elif not knee_bent:
                            if knee_joint_angle > 145.0:
                                global_telemetry['message'] = "Warning: Bend your knee more"
                            else:
                                global_telemetry['message'] = "Warning: Push foot further away"
                        else:
                            if tibia_table_angle < 43.0:
                                global_telemetry['message'] = "Warning: Pull foot closer to buttock"
                            else:
                                global_telemetry['message'] = "Warning: Push foot further away down"
