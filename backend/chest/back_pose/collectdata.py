...
OUTPUT_CSV = "chest_back_dataset.csv"
CAMERA_INDEX = 2  # External DroidCam
...
dataset = []
print("--- Chest BACK-POSE (PA View) Collection ---")
print("HOLD [r]: Collect Right Back Posture | HOLD [w]: Collect Wrong Posture | [q]: Save & Quit")

while True:
    ret, frame = cap.read()
    if not ret: break

    # We do NOT flip the frame during collection to ensure 
    # the ML model learns the raw coordinate space.
    h, w, _ = frame.shape
    ...
    status_msg = "IDLE - STAND WITH BACK TO CAMERA"

    if key == ord('r'):
        current_label, status_msg, hud_color = "Correct_Back_Pose", "RECORDING BACK POSE...", (0, 255, 0)
    elif key == ord('w'):
    ...
    cv2.putText(frame, f"{status_msg} | Samples: {len(dataset)}", (50, h-30), 
                cv2.FONT_HERSHEY_DUPLEX, 0.9, hud_color, 2)
    
    cv2.imshow("Back-Pose Collection", frame)
